...
std::vector<token_pair> Lex::getTokens(std::string fname)
{
	std::string code = readCodeFromFile(fname);
	std::vector<Lex::tokens> myRegex = getRegexFromFile();
	...
	while (!code.empty())
	{
		...
		std::string buff;
		std::vector<Lex::tokens> tempTokens;
		...
		if (code.size() != 0)
		{
			do
			{
				...
				if (tempTokens.empty())
				{
					...
					Lex::tokens tempResult = tempTokens[0];
...
std::vector<Lex::tokens> Lex::getRegexFromFile()
{
	std::vector<Lex::tokens> result;
	...
	else
	{
		while (!rxfile.eof())
		{
			...
			result.push_back(Lex::tokens{ RXname , RXrx , RXpr });
		}
		rxfile.close();
	}
	return result;
...
std::vector<Lex::tokens> Lex::findMatch(std::string str, std::vector<Lex::tokens> & myRegex)
{
...
void Lex::eraseWhitespaces(std::string & str)
{
	char space = ' ';
	char tab = '	';
	char newLine = '\n';
...
std::string Lex::readCodeFromFile(std::string fname)
{
